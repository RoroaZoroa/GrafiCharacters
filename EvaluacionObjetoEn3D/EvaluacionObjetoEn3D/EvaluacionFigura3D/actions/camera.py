...
def apply_camera():
    ...
    if state.camera_follow:
        # Interpolación suave de la cámara hacia Robot Espacial
        target_x = state.robot_x
        target_z = state.robot_z + 7.0  # Se posiciona detrás de él para verlo
        target_y = 2.0
        
        cam_pos[0] += (target_x - cam_pos[0]) * 0.08
        cam_pos[1] += (target_y - cam_pos[1]) * 0.08
        cam_pos[2] += (target_z - cam_pos[2]) * 0.08
        
        # Suavizar rotación hacia el frente
        yaw += (0.0 - yaw) * 0.1
        pitch += (0.0 - pitch) * 0.1
