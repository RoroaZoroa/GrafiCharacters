...
class CharacterManager:
    ...
    def update(self, dt):
        ...
        if hasattr(ajolote, 'state'):
            ...
            elif ajolote.state.movimiento == "caminando":
                # Animación de caminata para el Ajolote (oscilación de patas)
                ajolote.state.frame_animacion = math.sin(self.timers[0] * 15) * 30
            else:
                # Respiración suave
    ...
    def set_walking(self, index, walking):
        if index == 0: # Ajolote
            ...
            chef.get_chef().current_action = "CAMINAR" if walking else "IDLE"
        elif index == 2: # Knuckles
            self.knuckles_anim["walking"] = walking
        elif index == 3: # Mapache
            mapache.state.movimiento = "caminando" if walking else "quieto"
